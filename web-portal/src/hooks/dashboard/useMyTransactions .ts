...
const headings = {
  ...
  totalAmountRequiredBySeller: { label: "Amount", prefix: 'R' },
...
export function useMyTransactions(props:UseMyTransactionsProps){
    const { data, error, isLoading, onSearch } = useApplication(props);

    return { headings, data, onSearch, error, isLoading }
}

type UseMyTransactionsProps = UseApplicationProps 